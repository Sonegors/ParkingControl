...
import com.api.parkingcontrol.dtos.VagaDTO;
import com.api.parkingcontrol.models.ParkingSpotModels;
import com.api.parkingcontrol.services.VagaService;
...
public class VagaController {
    ...
    public ResponseEntity<Object> save(@RequestBody @Valid VagaDTO vagaDTO) {
        ...
        var vagaModel = new ParkingSpotModels();
    ...
    @GetMapping
    public ResponseEntity<Page<ParkingSpotModels>> getAll(@PageableDefault(page = 0, size = 10, sort = "id", direction = Sort.Direction.ASC) Pageable pageable) {
        return ResponseEntity.status(HttpStatus.OK).body(vagaService.findAll(pageable));
    }

    @GetMapping("/{id}")
    public ResponseEntity<Object> getById(@PathVariable(value = "id") UUID id) {
        Optional<ParkingSpotModels> vagaModelOptional = vagaService.findById(id);
